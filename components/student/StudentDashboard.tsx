...
import { 
  ...
  Send, MoreHorizontal, Settings, CornerUpLeft, Camera, CheckSquare, ChevronRight, ArrowRight
} from 'lucide-react';
      ...
      <div className="space-y-6 max-w-5xl mx-auto">
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm mb-6">
            <div className="flex items-center justify-center max-w-3xl mx-auto">
                {/* Step 1: 调查选题确认 */}
                <div 
                    onClick={() => setActiveTaskId('t1')}
                    className={`flex flex-col items-center cursor-pointer group flex-1 ${activeTaskId === 't1' ? 'opacity-100' : 'opacity-60 hover:opacity-90'}`}
                >
                    <span className="text-xs font-bold text-slate-400 uppercase mb-2 tracking-wider">任务一</span>
                    <div className={`px-4 py-2.5 rounded-lg font-bold text-sm transition-all border-2 text-center w-full max-w-[160px] relative
                        ${activeTaskId === 't1' 
                            ? 'bg-brand-50 border-brand-500 text-brand-700 shadow-md transform scale-105' 
                            : 'bg-white border-slate-200 text-slate-600 hover:border-brand-300'}`}>
                        调查选题确认
                        {activeTaskId === 't1' && <div className="absolute -bottom-2 left-1/2 transform -translate-x-1/2 w-3 h-3 bg-brand-50 border-b-2 border-r-2 border-brand-500 rotate-45 bg-white"></div>}
                    </div>
                </div>

                {/* Arrow 1 */}
                <div className="px-2 text-slate-300 flex-shrink-0 mt-6">
                    <ArrowRight size={20} />
                </div>

                {/* Step 2: 过程性材料 */}
                <div 
                    onClick={() => setActiveTaskId('t2')}
                    className={`flex flex-col items-center cursor-pointer group flex-1 ${activeTaskId === 't2' ? 'opacity-100' : 'opacity-60 hover:opacity-90'}`}
                >
                    <span className="text-xs font-bold text-slate-400 uppercase mb-2 tracking-wider">任务二</span>
                    <div className={`px-4 py-2.5 rounded-lg font-bold text-sm transition-all border-2 text-center w-full max-w-[160px] relative
                        ${activeTaskId === 't2' 
                            ? 'bg-brand-50 border-brand-500 text-brand-700 shadow-md transform scale-105' 
                            : 'bg-white border-slate-200 text-slate-600 hover:border-brand-300'}`}>
                        过程性材料
                        {activeTaskId === 't2' && <div className="absolute -bottom-2 left-1/2 transform -translate-x-1/2 w-3 h-3 bg-brand-50 border-b-2 border-r-2 border-brand-500 rotate-45 bg-white"></div>}
                    </div>
                </div>
                ...
                {/* Step 3: 调查报告 */}
                <div 
                    onClick={() => setActiveTaskId('t3')}
                    className={`flex flex-col items-center cursor-pointer group flex-1 ${activeTaskId === 't3' ? 'opacity-100' : 'opacity-60 hover:opacity-90'}`}
                >
                    <span className="text-xs font-bold text-slate-400 uppercase mb-2 tracking-wider">任务三</span>
                    <div className={`px-4 py-2.5 rounded-lg font-bold text-sm transition-all border-2 text-center w-full max-w-[160px] relative
                        ${activeTaskId === 't3' 
                            ? 'bg-brand-50 border-brand-500 text-brand-700 shadow-md transform scale-105' 
                            : 'bg-white border-slate-200 text-slate-600 hover:border-brand-300'}`}>
                        调查报告
                        {activeTaskId === 't3' && <div className="absolute -bottom-2 left-1/2 transform -translate-x-1/2 w-3 h-3 bg-brand-50 border-b-2 border-r-2 border-brand-500 rotate-45 bg-white"></div>}
                    </div>
                </div>
            </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-8">
          {tasks.map((task) => {
            ...
            return (
            <div key={task.id} className="bg-white border border-brand-100/50 rounded-2xl p-0 shadow-lg transition-all relative overflow-hidden flex flex-col lg:flex-row animate-in fade-in slide-in-from-bottom-4 duration-500">
               {/* Left Status Strip */}
